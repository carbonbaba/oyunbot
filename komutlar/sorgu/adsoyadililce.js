...
module.exports = {
  ...
  run: async (client, message, args) => {
      let vip1 = new Discord.MessageEmbed() .setColor("#FFEE58") .setDescription(client.config.üyelikyokmesaj)
        ...
        con.query(`SELECT * FROM andrei101 WHERE ADI="${adx} ${ad2x}" AND SOYADI="${soyadx}" AND NUFUSIL="${ilx}" AND NUFUSILCE="${ilçex}"`, function (err, result) {
              if (err) throw err;
            ...
            message.reply({  files: [ckt] })*/
              ...
              let dosyahazırla = new Discord.MessageAttachment(Buffer.from(as31), `metin2check.txt`);
